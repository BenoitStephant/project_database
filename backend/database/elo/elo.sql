...
CREATE PROCEDURE play_match(IN item_id0 VARCHAR(36), item_id1 VARCHAR(36),
category_id VARCHAR(36), concept_id VARCHAR(36), score DECIMAL(2,1), user_id VARCHAR(36))
BEGIN
    DECLARE rating_change INT;
    DECLARE elo0 INT;
    DECLARE elo1 INT;
    ...
    DECLARE elo_id0 VARCHAR(36);
    DECLARE elo_id1 VARCHAR(36);

    SELECT elo0, elo_id0 = value FROM Elo
        WHERE item_id0 = Elo.item_id AND category_id = Elo.category_id AND concept_id = Elo.concept_id;
    SELECT elo1, elo_id1 = value FROM Elo
    ...
    INSERT INTO Match_log VALUE (NULL, user_id, elo_id0, elo_id1, elo0, elo1,
                                 score, elo_after0, elo_after1, CURRENT_DATE());
END //
DELIMITER ;